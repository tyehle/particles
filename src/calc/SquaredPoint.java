...
/**
 * Represents a charged point which either attracts or repels particles.  This
 * implementation can be modeled by a charge floating above a plane which all
 * of the particles move on.  This means that as the particles get closer to the
 * point the force on them will go up according to the 1/x^2 relationship of
 * electrostatic forces, but at the same time will get smaller because less of
 * the force will be across the screen and more of it will be into or out of the
 * screen.  This has the effect of making the particle accelerate as it gets
 * closer to the point, but then at the last moment to just slide by, because
 * when the particle is directly over the point there is no force on it.  I am
 * hoping that this model will reduce the severity of rounding errors because
 * particles will not have as high forces on them as a point which is on the 
 * same plane as the particles.
 * @author Tobin
 * @version 4.1
 */
public class SquaredPoint extends StationaryPoint
{
    ...
    /**
     * Makes a new squared point with the given charge, height above the
     ...
     */
    public SquaredPoint(double charge, double h, CartesianVector position, boolean attract)
    {
        super(position);
        tempQ = q = charge;
        tempH = this.h = h;
     ...
     * each particle according to the following equation.  F = (d k q_1 q_2) / (h^2 + d^2)^(3/2)
    ...
    public void accelerateParticle(Particle p, double dt)
    {
        ...
        double denominator = Math.hypot(h, d);//root(h^2+d^2)
    ...
    /**
     * Makes a new instance of a squared point with all the same parameters as
     * this one.
    ...
    public Object clone()
    {
        return new SquaredPoint(q, h, new CartesianVector(position.x,
