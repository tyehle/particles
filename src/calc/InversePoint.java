 ...
 * @version 4.1
 */
public class InversePoint extends StationaryPoint
{
    ...
    /**
     * Makes a new inverse point with the given charge, height above the
     ...
     */
    public InversePoint(double charge, double h, CartesianVector position, boolean attract)
    {
        super(position);
        tempQ = q = charge;
        tempH = this.h = h;
     ...
     * each particle according to the following equation.  F = (d k q_1 q_2) / (h^2 + d^2)
    ...
    /**
     * Makes a new instance of an inverse point with all the same parameters as
     * this one.
    ...
    public Object clone()
    {
        return new InversePoint(q, h, new CartesianVector(position.x,
